...
router.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        if (!mongoose.Types.ObjectId.isValid(fieldId) || !mongoose.Types.ObjectId.isValid(userId)) {
        ...
        if (existingBookingsCount >= field.quantity) {
          return res.status(400).json({ message: "Field is fully booked for this date" });
        }
